// human-diff.v6.final.ts
// Human-Readable Diff Engine for Audit Logs
// 100% Generic • Zero Crash • Circular Safe • Production Ready
...
const getByPath = (obj: any, path: string[]): unknown => {
  ...
  for (const seg of path) {
    ...
    current = isArray(current) && /^\d+$/.test(seg) ? current[Number(seg)] : current[seg];
  }
  return current;
};
...
// ============================================================
// 4. Main Diff Engine – 100% Safe & Human Readable
// ============================================================
export const getNestedHumanDiff = <T extends object>(
  ...
  for (const op of patch) {
    ...
    for (let i = 0; i < rawParts.length; i++) {
      ...
      if (isIndex && (isArray(oldPtr) || isArray(newPtr))) {
        const idx = Number(part);
        const oldItem = isArray(oldPtr) ? oldPtr[idx] : undefined;
        const newItem = isArray(newPtr) ? newPtr[idx] : undefined;

        if (opts.arrayValueAsKeyForPrimitives && (oldItem != null || newItem != null)) {
          const bothPrimitive =
            (oldItem == null || typeof oldItem !== 'object') && (newItem == null || typeof newItem !== 'object');
          if (bothPrimitive) {
            const value = newItem ?? oldItem;
      ...
      if (i < rawParts.length - 1) {
        oldPtr = isObject(oldPtr) || isArray(oldPtr) ? oldPtr[part] : undefined;
        newPtr = isObject(newPtr) || isArray(newPtr) ? newPtr[part] : undefined;
      }
    }
  ...
  const walk = (node: unknown, currentPath: string, currentPathArray: string[]) => {
    ...
    if (isLeaf) {
      ...
      if (formatters[currentPath]) {
        try {
          if (old !== undefined) oldStr = formatters[currentPath](old);
          if (newVal !== undefined) newStr = formatters[currentPath](newVal);
        } catch {}
      }
...
export const getChangeSummary = (log: BaseLog, options: SummaryOptions = {}): string => {
  ...
  if (!log.old_data && log.new_data) changesText = ' → Created new record';
    ...
    if (changes.length === 0) changesText = ' (no changes detected)';
    else {
...
// ============================================================
// 7. Concise Test Runner (Production Grade)
// ============================================================
if (typeof require !== 'undefined' && require.main === module) {
  console.log('human-diff v6.final – Concise Test Suite\n');
  ...
  console.log('All tests passed! 100% safe, robust, and human-readable');
}
