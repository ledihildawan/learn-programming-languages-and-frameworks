...
import { useFormItem } from "./ui/form";
    ...
    <Popover open={open} onOpenChange={setOpen} modal={modal}>
      ...
      <PopoverContent className="w-auto p-2">
        ...
        <div className="relative overflow-hidden">
          ...
          />
          <div
            className={cn("absolute top-0 right-0 bottom-0 left-0", monthYearPicker ? "bg-popover" : "hidden")}
          ></div>
          <MonthYearPicker
            value={month}
            ...
            minDate={minDate}
            maxDate={maxDate}
            className={cn("absolute top-0 right-0 bottom-0 left-0", monthYearPicker ? "" : "hidden")}
          />
        </div>
...
function MonthYearPicker({
  ...
  const years = useMemo(() => {
    ...
    for (let i = minDate?.getFullYear() || 1912; i <= (maxDate?.getFullYear() || 2100); i++) {
      let disabled = false;
