...
export function SignInForm({
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const password = event.currentTarget.password.value?.trim() as string;
    const emailOrUsername = event.currentTarget.email.value?.trim() as string;
    ...
    const options = {
      ...
      fetchOptions: {
        ...
        onError: (data) => {
          toast.error(data.error.message || data.error.statusText);
    ...
    if (emailOrUsername.includes("@")) {
      options.email = emailOrUsername;

      authClient.signIn.email(options);
    } else {
      options.username = emailOrUsername;

      authClient.signIn.username(options);
    }
  }
